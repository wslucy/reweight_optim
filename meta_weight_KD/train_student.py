...
from models.util import ConvReg, SelfA, SRRL, SimKD, MadKD, MLP as MetaNet
...
from distiller_zoo import DistillKL, HintLoss, Attention, Similarity, VIDLoss, SemCKDLoss, DKDLoss, CSKDLoss, MadKDLoss
...
def parse_option():
    ...
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'hint', 'attention', 'similarity', 'vid',
                                                                      'crd', 'semckd','srrl', 'simkd', 'dkd', 'cskd',
                                                                      'madkd'])
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    if opt.distill == 'kd':
        ...
        criterion_kd = CSKDLoss()
        model_pseudo = model_dict[opt.model_s](num_classes=n_cls)
        model_meta = MetaNet()
        module_list.append(model_pseudo)
        module_list.append(model_meta)
        trainable_list.append(model_meta)
    ...
    if opt.distill == 'cskd': 
        # module_list = [model_s, model_pseudo, model_meta, model_t]
        # trainable_list = [model_s, model_meta]
        optimizer = optim.SGD((trainable_list[:1]).parameters(),
                            lr=opt.learning_rate,
                            momentum=opt.momentum,
                            weight_decay=opt.weight_decay)
        # according to the MW-Net paper
        optimizer_meta = optim.Adam(trainable_list[-1:].parameters(),
                            lr=1e-5,
                            weight_decay=0.)
    else:
    ...
    print(f"distill: {opt.distill}; a:{opt.cls}; d: {opt.div}; b: {opt.beta}")  
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        time1 = time.time()
        train_acc, train_acc_top5, train_loss = train(epoch, train_loader, module_list, criterion_list, optimizer, optimizer_meta, opt)
        time2 = time.time()
