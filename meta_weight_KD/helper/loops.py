...
from .util import AverageMeter, accuracy, reduce_tensor, MetaSGD
...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, optimizer_meta, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        # meta
        if opt.distill == 'cskd':
            pseudo_net = module_list[1]
            meta_net = module_list[2]
            pseudo_net.load_state_dict(model_s.state_dict())
            pseudo_outputs = pseudo_net(images)    
            with torch.no_grad():
                logit_t = model_t(images, is_feat=False)
            # 公式3 构建计算图，利用meta_net更新pseudo_net
            meta_delta, meta_lambda = meta_net(torch.cat([pseudo_outputs, logit_t], dim=1))
            pseudo_loss = criterion_kd(pseudo_outputs, logit_t, meta_delta, meta_lambda, labels, epoch)
            pseudo_grads = torch.autograd.grad(pseudo_loss, pseudo_net.parameters(), create_graph=True)
            # 更新pseudo_net
            pseudo_optimizer = MetaSGD(pseudo_net, pseudo_net.parameters(), lr=optimizer.param_groups[0]['lr'])
            ...
            # 公式4 更新meta_net，
            # 获取元数据
            meta_inputs, meta_labels = next(iter(train_loader))
            if opt.gpu is not None:
                meta_inputs = meta_inputs.cuda(opt.gpu if opt.multiprocessing_distributed else 0, non_blocking=True)
            if torch.cuda.is_available():
                meta_labels = meta_labels.cuda(opt.gpu if opt.multiprocessing_distributed else 0, non_blocking=True)
            meta_outputs = pseudo_net(meta_inputs)
            with torch.no_grad():
                meta_logit_t = model_t(meta_inputs, is_feat=False)
            meta_loss = criterion_cls(meta_outputs, meta_labels.long())
            # meta_loss += criterion_div(meta_outputs, meta_logit_t)
        ...
        if opt.distill == 'kd':
            ...
            loss_kd = criterion_kd(logit_s, logit_t, labels, epoch)
        elif opt.distill == 'madkd':
            y_s_model, y_t_model, y_s_mlp_rev, y_t_mlp_rev = module_list[1](logit_s, logit_t, opt.rev_factor, labels)
            loss_kd = criterion_kd(y_s_model, y_t_model, y_s_mlp_rev, y_t_mlp_rev, labels, epoch)
        ...
        elif opt.distill == 'cskd':
            # 公式5 正常的训练过程。利用更新好的meta_net，计算损失（过程中不需要meta_net的梯度）
            with torch.no_grad():
                delta, lambda_ = meta_net(torch.cat([logit_s, logit_t], dim=1))
            loss_kd = criterion_kd(logit_s, logit_t, delta, lambda_, labels, epoch)
        else:
        ...
        if opt.distill == 'madkd':
            torch.nn.utils.clip_grad_norm_(module_list[1].parameters(), 1)
        optimizer.step()        
